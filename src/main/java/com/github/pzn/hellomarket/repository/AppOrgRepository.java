...
public interface AppOrgRepository extends CrudRepository<AppOrg, Long> {
  ...
  @Query("SELECT ao FROM AppOrg ao JOIN FETCH ao.appUsers")
  Iterable<AppOrg> findAllFetchUsers();

  @Query("SELECT ao FROM AppOrg ao JOIN FETCH ao.appUsers WHERE ao.code = :code")
